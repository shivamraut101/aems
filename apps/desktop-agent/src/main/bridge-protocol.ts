// AEMS BRIDGE PROTOCOL — the desktop agent's copy.
//
// `apps/browser-extension/src/protocol.ts` holds the other copy, and everything below
// the marker line is byte-identical between the two. `bridge-protocol.test.ts` reads
// both files and fails if they ever drift, because the two are compiled by different
// toolchains — the agent's by electron-vite, the extension's by tsc into an MV3
// service worker — and neither can import from the other's build.
//
// Keep the shared half free of imports for that reason.
